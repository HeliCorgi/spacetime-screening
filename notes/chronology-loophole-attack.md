...
作業開始時の読取基点はPR #11の `ad05b9b59a595765a822f7c18155820bdafbf1dc`。公開前に同PRのマージを確認したため、提出基点はmainの `20be82377c956c0b4e4457babd4f6058fd5b0d20` とする。
[前回の場・検出器ノート](taubnut-field-detector-closure.md)と[KRW監査](operational-past-signalling-focus.md)を引き継ぐ。
今回の追加は別の通常PRとして提出し、mainは変更しない。既存の計算は取り消さない。
...
既存code・assert・参照JSON・依存・CI・Leanは変更しない。新規2本は独立script。前回の2本はmainへマージ済みで変更がないため、今回は新規2本を既存のPR累積差分選別器で検査し、対象と理由を記録する。
