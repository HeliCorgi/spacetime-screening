...
2. the exact map between target \(t\)-winding and coset/gauge quantum numbers;
...
\[
\boxed{
\textbf{State-map gate:}
\quad
\text{identify the exact coset/BRST quantum number that represents
chronology-fiber winding.}
}
\]
...
Do not recompute the exact target geometry.
...
and determine the exact gauged-WZW/BRST realization of a sector with target
monodromy
...
Keep the work timeout-safe:

1. first derive the two \(U(1)\) gauge-charge constraints on a candidate
   affine primary;
2. separately derive the large-\(U(1)_B\) sector/winding identification;
3. only then combine with Virasoro/BRST conditions;
4. stop immediately if the chronology state is absent/nonunitary/
   nonnormalizable;
5. otherwise proceed to a small minisuperspace crossing calculation before any
   large exact-correlator calculation.
