...
**Status:** Phase III chronology side branch.
...
2. the **quantization** of the now-identified combined \(U(1)_B\) monodromy
   sector and its exact affine zero-mode/conformal-weight shifts;
...
The next calculation should begin **before** a large correlator computation.

The classical/gauge part of the state map is now partially resolved.

The first unresolved problem is:

\[
\boxed{
\textbf{Twisted-sector quantization gate:}
\quad
\text{quantize the combined }U(1)_B\text{ monodromy generated by }
\epsilon_B=4\pi w.
}
\]
...
1. construct a candidate vertex operator in the parent
   \(SL(2,\mathbb R)\times SU(2)\) affine theory;
2. impose both gauged \(U(1)\) charge constraints;
3. include the heterotic left-moving charge contribution required by anomaly
   cancellation;
4. identify the large-\(U(1)_B\) topological/holonomy sector corresponding
   semiclassically to
   \[
   t(\sigma+2\pi)-t(\sigma)=4\pi\lambda w;
   \]
5. check BRST closure, non-exactness, norm, level matching, and
   normalizability;
6. only then attempt a horizon-crossing two-point/reflection observable.

If the required state disappears from BRST cohomology, becomes negative norm,
or is nonnormalizable, this phase should stop there and record a model-specific
chronology-protection mechanism.

If it survives, the next step is minisuperspace/full-CFT crossing.
...
Do not recompute the metric, wrapped determinant, or large-gauge
identification.
...
\[
\boxed{
\text{Twisted-sector quantization gate}.
}
\]

The target monodromy is now known at the parent-field level:
...
\[
\Delta\Phi_1=4\pi w(Q_B+\lambda),
\qquad
\Delta\Phi_2=4\pi w(P_B+1).
\]

Next:

1. derive the induced mode/zero-mode shifts of the embedded right
   \(SL(2,\mathbb R)\) and \(SU(2)\) currents;
2. include the left heterotic charge contribution fixed by
   \(Q_{A,B},P_{A,B}\);
3. verify the two gauge constraints in the twisted sector;
4. derive \(L_0,\bar L_0\) and level matching;
5. test the unitary \(SL(2,\mathbb R)\) representation range,
   normalizability, and BRST non-exactness;
6. stop if the state fails any of those gates;
7. only if it survives, formulate the smallest Taub-to-NUT transmission
   observable.

The BTZ hyperbolic-spectral-flow literature is methodological prior art, not a
substitute for deriving the Taub--NUT asymmetric heterotic twist.
