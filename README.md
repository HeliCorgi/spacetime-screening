...
>
> **今回の判定：C。** #25の二入力逆散乱を最大限有利に認めても、同じ固定・線形・operation-independent な量子過程として閉じ、過去の受信記録を普通に未来へ保存して copy / NOT を許すと、非零の過去識別度は正規化と両立しません。

| 明示した要素 | 結果 |
|---|---|
| **未来側 `do(0)/do(1)`** | 二入力 `F_A=(-1)^b T* g`, `F_B=-(-1)^b R g` で #25 の target 出力を作る |
| **量子状態** | 共通 Gaussian seed＋bit反転 coherent displacement。準備雑音も `V_eff` に含める |
| **有限受信器** | Ramsey qubit、有限 smearing、`sigma_y` 全結果を保存 |
| **開いた受信則** | `P(Y=y|do(b))=[1+y(-1)^b d]/2`, `D_past=|d|` |
| **閉路の copy / NOT** | 全履歴重みが `1+d` / `1-d`。同じ物理過程が両方で正規化されるには `d=0` |
| **最終分布** | `P(Y|do(0))=P(Y|do(1))=(1/2,1/2)`, **`D_past=0`** |

最終成果物は **`P(Y|do(0))=(1/2,1/2)`、`P(Y|do(1))=(1/2,1/2)`、`D_past=0`**。これは未計算値を0で埋めたものではなく、copy/NOTを含む同一のoperation-independent線形過程の正規化から導いたC判定です。

未計算量を0で埋めたのではありません。開いた模型が `D>0` を主張した場合、閉路の正規化欠陥が **ちょうど `D`** になり、固定no-signal kernelへ戻す最小の各column TV修正は `D/2` です。future operationごとのpostselectionや境界条件変更は、このCからの前提変更であり現行Aではありません。

**[全導出・量子状態・散乱・受信確率・Cの適用範囲](notes/sa-closed-operational-loop-v6.md)** ／ [順方向計算](src/symbolic/sa_closed_loop_v6.py) ／ [独立検算](src/symbolic/sa_closed_loop_verify_v6.py)
