#!/usr/bin/env python3
"""Direct l=2 odd-harmonic expansion of the original A-B action.
...
Odd l=2,m=0 Regge-Wheeler-gauge perturbations:
...
assert sp.simplify(A_EH - 12 * sp.pi / 5) == 0
assert sp.simplify(C_QW - 48 * sp.pi / (5 * r)) == 0
...
H_expected = sp.factor(24 * sp.pi / 5) * sp.Matrix(
    [
        [1, -4 * ell**2 * a / f, +4 * ell**2 * b / f],
        [-4 * ell**2 * a / f, 0, 0],
        [+4 * ell**2 * b / f, 0, 0],
...
def main() -> None:
    print("== Direct original-action l=2 odd expansion ==")
